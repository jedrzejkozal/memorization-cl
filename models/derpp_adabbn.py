...
from backbones.utils.adab2n import AdaB2N
...
class DerppAdaBBN(ContinualModel):
    NAME = "derpp_adabbn"
    ...
    def __init__(self, backbone, loss, args, transform):
        ...
        self.norm_modules = [m for m in self.net.modules() if isinstance(m, BatchNorm2d)]
        assert all(isinstance(m, AdaB2N) for m in self.norm_modules)
    ...
    def set_counts(self, labels):
        ...
        task_counts_extended = labels.new_zeros(self.task + 1)  # in case a task not in batch
    ...
    def observe(self, inputs, labels, not_aug_inputs):
        ...
        if not self.buffer.is_empty():
            buf_inputs1, buf_labels1, buf_logits1 = self.buffer.get_data(self.args.minibatch_size, transform=self.transform)
            buf_inputs2, buf_labels2, _ = self.buffer.get_data(self.args.minibatch_size, transform=self.transform)
            ...
            if self.task >= self.args.ada_t0:
                loss_norm = (torch.stack([m.loss for m in self.norm_modules]).sum() * self.args.adab2n_lambd)
            else:
                loss_norm = torch.tensor(0.0, device=inputs.device)

            loss = self.loss(outputs, labels)
            loss += self.args.alpha * F.mse_loss(buf_outputs1, buf_logits1)
            loss += self.args.beta * self.loss(buf_outputs2, buf_labels2)
            loss += loss_norm
        else:
