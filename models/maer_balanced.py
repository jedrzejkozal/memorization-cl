...
class MaerBalanced(ContinualModel):
    ...
    def end_task(self, dataset):
        ...
        if self.args.buffer_policy == 'max':
            ...
            selected_samples_idxs = self.trained_order[int(0.1*len(self.trained_order)):][:data_size]
        elif self.args.buffer_policy == '0.9max':
            selected_samples_idxs = self.trained_order[:int(0.9*len(self.trained_order))][-data_size:]
            ...
            selected_samples_idxs = np.random.choice(trained_order, size=data_size, replace=False, p=selection_prob).tolist()
        ...
        if self.args.use_original_memscores:
            ...
            if self.args.buffer_policy == '0.1min':
                selected_samples_idxs = arg_idx[int(0.1*len(arg_idx)):][:data_size]
            elif self.args.buffer_policy == 'min':
                selected_samples_idxs = arg_idx[:data_size]
            elif self.args.buffer_policy == 'max':
                selected_samples_idxs = arg_idx[-data_size:]
            elif self.args.buffer_policy == 'middle':
                half_idx = len(arg_idx) // 2
                select_size = data_size // 2
