...
def permute_network(train_aug_loader, source_network, premuted_network, device, epochs=1):
    ...
    for k in range(len(blocks1)):
        ...
        if k+1 == len(blocks1) and type(premuted_network.classifier) == nn.Identity:
            break

        if k > 0:
            permute_output(perm_map, blocks1[k].conv2, blocks1[k].bn2)
            if hasattr(blocks1[k], 'shortcut'):
        ...
        if k+1 < len(blocks1):
            ...
            if hasattr(blocks1[k+1], 'shortcut'):
...
def add_junctures(net, device):
    ...
    for block in blocks:
        if not hasattr(block, 'shortcut') or type(block.shortcut) != nn.Sequential or len(block.shortcut) > 0:
            continue
        planes = len(block.conv2.weight)  # len(block.bn2.weight)
...
def get_blocks(net):
    if hasattr(net, 'layer4'):
        blocks = nn.Sequential(nn.Sequential(net.conv1, net.bn1), *net.layer1, *net.layer2, *net.layer3, *net.layer4)
    else:
        blocks = nn.Sequential(nn.Sequential(net.conv1, net.bn1), *net.layer1, *net.layer2, *net.layer3)
    return blocks
