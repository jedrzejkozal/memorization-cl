...
def main():
    ...
    table = list()
    algorithms = ['bottom-k memscores', 'high bottom-k', 'middle-k memsocres', 'high middle-k']
    for algorithm_name in algorithms:
        row = list()
        ...
        for a, a_s in zip(accs, acc_std):
            a = round(a, 2)
            a_s = round(a_s, 2)
            acc_str = f'{a}±{a_s}'
            row.append(acc_str)

        if row[0].startswith('high'):
            previos_row = table[-1]
            for i in range(1, 5):
    ...
    tab_latex = tabulate.tabulate(table, tablefmt="latex", headers=['method', '2000', '5000', '10000', '20000'])
